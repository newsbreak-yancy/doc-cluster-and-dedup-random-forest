...
import com.nb.randomforest.utils.FeatureUtils;
import com.nb.randomforest.utils.NEFeatureUtils;
import com.nb.randomforest.utils.SpacyFeatureUtils;
...
public class EventFeature {
	...
	public static final FeatureUtils NeFeatureUtils = new NEFeatureUtils();

	public static final FeatureUtils SpacyFeatureUtils = new SpacyFeatureUtils();
    ...
    public EventFeature(JsonNode masterNode, JsonNode canditNode, String label, TokenizerType tokenizer) throws ParameterException {
    	if (!StringUtils.isEmpty(label) && !label.equals("DIFF") && !label.equals("EVENT") && !label.equals("DUP")) {
    		throw new ParameterException("Invalid Label! Required DIFF, EVENT , DUP or null.");
	    }

		FeatureUtils featureUtils = tokenizer == TokenizerType.NE ? NeFeatureUtils : SpacyFeatureUtils;
	    ...
	    this.epochSpan = featureUtils.numSpan(getEpoch(masterNode), getEpoch(canditNode));
	    this.parghSpan = featureUtils.numSpan(masterNode.get("paragraph_count"), canditNode.get("paragraph_count"));
	    this.simhashDist = featureUtils.simhashDist(masterNode.get("simhash"), canditNode.get("simhash"));
	    this.kwsRatio = featureUtils.overlapRatio(masterNode.get("kws"), canditNode.get("kws"));
	    this.channelRatio = featureUtils.overlapRatio(getChannels(masterNode), getChannels(canditNode));
	    
		if (tokenizer == TokenizerType.NE){
			this.cOrgOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("ne_content_organization"), canditNode.get("ne_content_organization"));
			this.cLocOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("ne_content_location"), canditNode.get("ne_content_location"));
			this.cPrsOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("ne_content_person"), canditNode.get("ne_content_person"));
			this.tOrgOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("ne_title_organization"), canditNode.get("ne_title_organization"));
			this.tLocOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("ne_title_location"), canditNode.get("ne_title_location"));
			this.tPrsOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("ne_title_person"), canditNode.get("ne_title_person"));
		}
		else{
			this.cOrgOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("spacy_content_org"), canditNode.get("spacy_content_org"));
			this.cLocOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("spacy_content_loc"), canditNode.get("spacy_content_loc"));
			this.cPrsOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("spacy_content_per"), canditNode.get("spacy_content_per"));
			this.tOrgOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("spacy_title_org"), canditNode.get("spacy_title_org"));
			this.tLocOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("spacy_title_loc"), canditNode.get("spacy_title_loc"));
			this.tPrsOverlapRatio = featureUtils.weightedOverlapRatio(masterNode.get("spacy_title_per"), canditNode.get("spacy_title_per"));
		}
	    
	    this.catOverlapRatio = featureUtils.categoryOverlapRatio(getCategory(masterNode), getCategory(canditNode));
	    this.geoOverlapRatio = featureUtils.geotagOverlapRatio(getGeoTag(masterNode), getGeoTag(canditNode));
    }
	...
	private static JsonNode getChannels(JsonNode node){
		return getField(node, "channels_v2", "channels");
	}

	private static JsonNode getCategory(JsonNode node){
		return getField(node, "text_category_v2", "text_category");
	}

	private static JsonNode getGeoTag(JsonNode node){
		return getField(node, "geotag_v2", "geotag");
	}

	private static JsonNode getField(JsonNode node, String preferredFieldName, String backupFieldName){
		JsonNode fieldNode = node.get(preferredFieldName);
		if (fieldNode == null){
			fieldNode = node.get(backupFieldName);
		}

		return fieldNode;
	}
	
	private static JsonNode getEpoch(JsonNode node){
		JsonNode epochNode = node.get("epoch");
		if (epochNode != null){
			if (!epochNode.isNumber()){
				epochNode = epochNode.get("$numberLong");
			}
		}
	...
	private static String getSegTitle(JsonNode node){
		return getFieldValue(node, "stitle", "seg_title");
	}

	private static String getFieldValue(JsonNode node, String preferredFieldName, String backupFieldName){
		if (node.hasNonNull(preferredFieldName)){
			return node.get(preferredFieldName).textValue();
		}
		else if (node.hasNonNull(backupFieldName)){
			return node.get(backupFieldName).textValue();
		}
	...
	public static void main(String[] args) throws Exception {
		...
		EventFeature feature = new EventFeature(mNode, cNode, "", TokenizerType.NE);
